/**
 * Official Chess.com Stockfish 16 + Gemini AI Overlay Extension (Manifest V3)
 * Animation Stability Guard + Strict Turn Validation
 */

console.log('⚡ Stockfish 16 + Gemini AI Extension V4 Active');
...
setInterval(() => {
  try {
    ...
    // IF OPPONENT'S TURN: Clear arrows immediately and do NOT draw!
    if (!isYourTurn) {
      if (svgOverlay) svgOverlay.innerHTML = '';
    ...
    // Stability delay guard: wait 200ms to ensure piece sliding animation has completely finished
    if (pendingFen !== currentFen) {
      pendingFen = currentFen;
      if (stableTimer) clearTimeout(stableTimer);
      stableTimer = setTimeout(() => {
        chrome.runtime.sendMessage({ action: 'evaluateFen', fen: currentFen }, (response) => {
